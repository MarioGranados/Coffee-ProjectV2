...
function renderCoffee(coffee) {
    let html = '<div class="coffee card-body">';
    html += '<div class="card-body border">'
    html += '<h3>' + coffee.name + '</h3>';
    html += '<p class="roast text-muted">' + coffee.roast + '</p>';
    if (favorite.id === 0) {
        html += '<i class="far fa-star"></i> <button data-id="' + coffee.id + '" class="btn btn-info favoriteButton m-2">Mark Favorite</button>';
    } else if (coffee.id !== favorite.id) {
        html += '<i class="far fa-star"></i> <button data-id="' + coffee.id + '" class="btn btn-info favoriteButton">Mark Favorite</button>';
    } else {
        html += '<i class="fas fa-star"></i> <button data-id="' + coffee.id + '" class="btn btn-warning" id="removeFavoriteButton">Remove Favorite</button>';
    }
    html += '<button data-id="' + coffee.id + '" class="btn btn-secondary editButton m-2">' + "Edit Coffee" + '</button>';
    html += '<button data-id="' + coffee.id + '" class="btn btn-danger deleteButton m-2" >' + "Delete Coffee" + '</button>';
    html += '<form>';
    html += '<label>Change Name <input style="text-align: center" class="editName form-control" data-id="' + coffee.id + '" type="text" value="' + coffee.name + '"></label>';
    html += '<label>Change Roast <select style="text-align: center" class="editRoast form-control" data-id="' + coffee.id + '"><option>(light roast)</option><option>(medium roast)</option><option>(dark roast)</option></select></label>';
    html += '</form>';
    html += '</div>';
    html += '</div>'
    return html;
